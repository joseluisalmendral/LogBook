...
globalThis.__LB_ESM_DIRNAME = __lbDirname(globalThis.__LB_ESM_FILENAME);`,
  ...
  // Slice 19: the PDF bundle entry was removed along with src/export/pdf.ts and
  // the `logbook export pdf` CLI command. PDF was a thin wrapper over
  // instructor-pack + puppeteer; both got deleted with the legacy shell.
  // Users who need PDF output can print-to-PDF from the new interactive HTML.
]);
