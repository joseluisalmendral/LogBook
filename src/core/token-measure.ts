/**
 * token-measure.ts — Fixed-context token counting for installed LogBook artifacts.
 *
 * Extracted from src/cli/commands/doctor.ts (iter6 T1).
 * Imported by:
 *   - src/cli/commands/doctor.ts (CLI doctor --measure command)
 *   - src/tui/persist.ts (TUI home screen token budget bar — iter6 T5)
 *
...
// ---------------------------------------------------------------------------
export const MCP_TOOL_DESCRIPTIONS: readonly string[] = [
...
export const SESSION_START_CONSERVATIVE_MAX_TOKENS = 120;
...
/**
 * Per-artifact-kind token breakdown for a given manifest.
 * All values are non-negative integers.
 */
export interface TokenBreakdown {
  /** SKILL.md only (Math.ceil(content.length / 4)). reference.md = 0 (on-demand). */
  skill: number;
  /** Math.ceil(blockBody.length / 4) for the installed augment block. */
  augmentClaudemd: number;
  /** Sum of Math.ceil(desc.length / 4) per MCP tool description (static constant). */
  mcpToolDescriptions: number;
  /** Sum of Math.ceil(description.length / 4) per slash command YAML description field. */
  slashCommandDescriptions: number;
  /** Always 0 — subagent descriptions are in the UI index, not agent context. */
  subagentDescriptions: number;
  /** Always 0 — statusline is a UI element, not injected into agent context. */
  statusline: number;
  /** 120 when a SessionStart hook is installed; 0 otherwise. Conservative max. */
  sessionStart: number;
}
...
/**
 * Compute the fixed-context token breakdown for an installed manifest.
 *
 * Reads installed files from `projectRoot` to measure actual content sizes.
 * All reads are synchronous. Missing files contribute 0 tokens (graceful fallback).
 *
 * @param manifest - The installed LogBook manifest.
 * @param projectRoot - Absolute path to the project root where artifacts are installed.
 * @returns TokenBreakdown with per-kind token counts.
 */
export function computeTokenBreakdown(manifest: Manifest, projectRoot: string): TokenBreakdown {
  ...
  for (const entry of manifest.artifacts) {
    ...
    if (entry.kind === "augment_claudemd") {
      ...
      const body = readAugmentBody(projectRoot, entry.file_path);
      if (body !== null) {
      ...
      const desc = readSlashDescription(projectRoot, entry.file_path);
      if (desc !== null) {
      ...
      if (basename === "SKILL.md") {
        const absPath = path.join(projectRoot, entry.file_path);
        let content: string;
        try {
...
// Re-export unused constants for internal backwards compat (START_MARKER / END_MARKER used in tests if any).
