/**
 * logbook session outcome <text> — Record a session outcome.
 *
 * Persists a user_entry event with entryType "session_outcome" tied to the
 * current session. Later writes for the same session win (latest-write-wins
 * per ADR-7). Sessions HTML renders the last outcome below the timeline.
 *
...
export default defineCommand({
  meta: {
    name: "outcome",
    description: "Record the outcome of the current session",
  },
  args: {
    text: {
      type: "positional",
      required: true,
      description: "Session outcome text (1–500 chars)",
    },
  },
  async run({ args }) {
    ...
    // Validate length.
    if (text.length === 0) {
      process.stderr.write("error: outcome text must not be empty\n");
      process.exit(1);
    }
    if (text.length > 500) {
      process.stderr.write(
        `error: outcome text is too long (${text.length} chars, max 500)\n`,
      );
      process.exit(1);
    }
